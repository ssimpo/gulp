...
function makeArray(ary) {
	...
	if ((ary === null) || (ary === undefined) || Number.isNaN(ary)) return [];
	if (ary instanceof Set) return [...ary.values()];
	return [ary];
}

function _getSurroundingJsonSymbols(txt) {
	const [firstCurly, firstSquare] = [txt.indexOf(jsonObjectSymbols[0]), txt.indexOf(jsonArraySymbols[0])];

	if (~firstCurly && !~firstSquare) return jsonObjectSymbols;
	if (~firstSquare && !~firstCurly) return jsonArraySymbols;
	if (firstCurly < firstSquare) return jsonObjectSymbols;
	return jsonArraySymbols;
}

// @note Refactored version of: https://stackoverflow.com/questions/10574520/extract-json-from-text
function extractJson(txt) {
	const [opener, closer] = _getSurroundingJsonSymbols(txt);

	let firstOpen, firstClose, candidate;
	firstOpen = txt.indexOf(opener, firstOpen + 1);

	do {
		firstClose = txt.lastIndexOf(closer);
		if (firstClose <= firstOpen) return null;
		do {
			candidate = txt.substring(firstOpen, firstClose + 1);
			try {return JSON.parse(candidate);} catch(err) {}
			firstClose = txt.substr(0, firstClose).lastIndexOf(closer);
		} while(firstClose > firstOpen);
		firstOpen = txt.indexOf(opener, firstOpen + 1);
	} while(firstOpen != -1);
}

lodash.mixin(lodash, {filesInDirectory, parseParameters, substituteInObj, forEach2, makeArray}, {chain:true});
lodash.mixin(lodash, {replaceSequence, substitute, extractJson});
