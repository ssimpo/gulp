...
const {isString, kebabCase, chain, get, substituteInObj} = require('./util');
...
const resolvers = new Set([
	...
	({param, cwd, inject, require})=>{
		if (param === 'settings') return importSettings(cwd, require);
...
function getJsonRequire(dirname, filename, localRequire=require) {
	try {
...
function importSettings(cwd, localRequire=require) {
	return substituteInObj(Object.assign(...[...settings.get('settings-paths').values()].map(settingsPath=>{
