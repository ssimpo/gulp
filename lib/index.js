...
function _createTask(taskId, tasks, gulp, registry) {
	...
	const taskFn = function(done) {
		if (!!getSetting('debug', registry) || !!getSetting('show-task-path', registry)) {
			log(`Found task '${taskId}' in`, task.path);
		}
		...
		const _task = function(__done) {
			...
			const di = getInjection({
				func:task.fn,
				cwd,
				path:(task.path || cwd),
				inject: {...(getSetting('inject', registry) || {}), gulp, done:complete}
			});
...
function importTasks(root, gulp=require('gulp')) {
	if (isObject(root)) return _addTasksToGulp(createTasks(root.root, root), root.gulp || gulp, root);
